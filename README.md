...
### 🏋️ Gesundheit
Sync-Ziel für die iPhone-App **Auto Health Export**: eigener API-Key pro User (getrennt vom Login), idempotenter Import (`POST /api/health/import`) für Vitalwerte, Blutdruck/-zucker, Schlaf und Workouts. Optionaler AI-Fallback für unbekannte Metriknamen. Manueller Nachimport per CSV oder JSON möglich. Frontend mit Dashboard, Vitalwert-Charts, Schlafphasen und Workout-Historie.
